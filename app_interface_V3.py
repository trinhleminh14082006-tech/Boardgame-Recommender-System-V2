...
# 🛠️ TÍNH NĂNG ĐÓNG/MỞ CỘT CHO GAME OUTPUT (BẢNG KẾT QUẢ)
st.sidebar.markdown("---")
...
if selected_game != "":
    ...
    for i, cluster_idx in enumerate(cluster_indices):
        ...
        # Thu thập đầy đủ dữ liệu thuộc tính nâng cao phục vụ cho Output hiển thị động
        results.append({
            ...
            'Mechanic': get_safe_val(df, cluster_idx, 'mechanic'),
            'Domain': get_safe_val(df, cluster_idx, 'domain'),
            'Category': get_safe_val(df, cluster_idx, 'category'),
            'Designer': get_safe_val(df, cluster_idx, 'designer'),
            'Artist': get_safe_val(df, cluster_idx, 'artist')
        })
    ...
    # 🛠️ THIẾT KẾ ĐÓNG/MỞ THÔNG TIN CHI TIẾT CHO GAME INPUT QUA CÁC EXPANDER
    ...
    with exp_col2:
        with st.expander("⚙️ Cơ Chế & Phân Loại"):
            mech = get_safe_val(df, idx, 'mechanic')
            cat = get_safe_val(df, idx, 'category')
            st.write(f"**Mechanic:**\n{mech}")
            st.write(f"**Category:**\n{cat}")
            
    with exp_col3:
        with st.expander("🌐 Hệ Sinh Thái (Domain)"):
            dom = get_safe_val(df, idx, 'domain')
            st.write(f"**Domain:**\n{dom}")
            
    with exp_col4:
        with st.expander("🎨 Tác Giả & Họa Sĩ"):
            des = get_safe_val(df, idx, 'designer')
            art = get_safe_val(df, idx, 'artist')
            st.write(f"**Designer:**\n{des}")
            st.write(f"**Artist:**\n{art}")
